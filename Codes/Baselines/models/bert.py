    ...
    def train(self, logger = None):
        ...
        if self.config['model_name'] == 'bert':
            self.model = BERTModel(
                self.config['emb_dim'],
                self.config['model']['mlp']['dims'],
                self.config['model']['mlp']['dropout'],
                self.config['bert_path']
            )

        elif self.config['model_name'] == 'qwen_image':
        ...
        elif self.config['model_name'] == 'qwen_vl':
        ...
        elif self.config['model_name'] == 'clip_image':
            from .clip_image import CLIPImageModel

            self.model = CLIPImageModel(
                self.config['emb_dim'],
                self.config['model']['mlp']['dims'],
                self.config['model']['mlp']['dropout']
            )
        elif self.config['model_name'] == 'blip_image':
            from .blip_image import BLIPImageModel

            self.model = BLIPImageModel(
                self.config['emb_dim'],
                self.config['model']['mlp']['dims'],
                self.config['model']['mlp']['dropout']
            )
        elif self.config['model_name'] == 'clip_vl':
            from .clip_vl import CLIPVLModel

            self.model = CLIPVLModel(
                self.config['emb_dim'],
                self.config['model']['mlp']['dims'],
                self.config['model']['mlp']['dropout']
            )
        elif self.config['model_name'] == 'blip_vl':
            from .blip_vl import BLIPVLModel

            self.model = BLIPVLModel(
                self.config['emb_dim'],
                self.config['model']['mlp']['dims'],
                self.config['model']['mlp']['dropout']
            )
