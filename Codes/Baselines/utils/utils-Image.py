...
def metrics(y_true, y_pred):
    ...
    rec_vals = recall_score(y_true, y_pred, average=None)
    pre_vals = precision_score(y_true, y_pred, average=None)
...
def process_test_results(test_file_path, test_res_path, label, pred, id, ae, acc):
    ...
    for index in range(len(label)):
        ...
        # --- pandas 1.x/2.x compatible iteration over a Series row ---
        if hasattr(cur_data, "items"):         # pandas >= 0.25 uses .items() on Series
            iterator = cur_data.items()
        elif hasattr(cur_data, "iteritems"):   # very old pandas fallback
            iterator = cur_data.iteritems()
        else:
            raise TypeError(f"Unsupported type for cur_data: {type(cur_data)}")
